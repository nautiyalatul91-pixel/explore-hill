-- =========================================================================
-- PART 2: CREATE TABLES, COLUMNS, TRIGGERS, STORAGE & SEED DATA
-- Run this AFTER Part 1 has been executed successfully.
-- =========================================================================

-- 1. Create updated_at trigger helper function if not exists
...
-- 3. Create permission checking helper function
...
-- Create trigger on packages if not exists
...
-- 5. Create package_itineraries table
...
-- 14. Create package_gallery table
...
-- 15. Extend existing bookings table safely
...
-- 16. Extend existing departures table safely
...
-- 17. Sync booking to customer trigger
...
DROP TRIGGER IF EXISTS trigger_sync_booking_to_customer ON public.bookings;
CREATE TRIGGER trigger_sync_booking_to_customer
  AFTER INSERT ON public.bookings
  FOR EACH ROW
  EXECUTE FUNCTION public.sync_booking_to_customer();

-- 18. Re-create fixed sync_departure_seats trigger function
...
-- 19. Table Row Level Security Policies
DROP POLICY IF EXISTS "Super admins can manage permissions" ON public.role_permissions;
CREATE POLICY "Super admins can manage permissions" ON public.role_permissions FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'super_admin'));

DROP POLICY IF EXISTS "Anyone can select permissions" ON public.role_permissions;
CREATE POLICY "Anyone can select permissions" ON public.role_permissions FOR SELECT TO anon, authenticated
  USING (true);

DROP POLICY IF EXISTS "Anyone can view packages" ON public.packages;
CREATE POLICY "Anyone can view packages" ON public.packages FOR SELECT TO anon, authenticated
...
DROP POLICY IF EXISTS "Content managers can insert packages" ON public.packages;
CREATE POLICY "Content managers can insert packages" ON public.packages FOR INSERT TO authenticated
...
DROP POLICY IF EXISTS "Content managers can update packages" ON public.packages;
CREATE POLICY "Content managers can update packages" ON public.packages FOR UPDATE TO authenticated
...
CREATE POLICY "Content managers can delete packages" ON public.packages FOR DELETE TO authenticated
  USING (public.has_permission(auth.uid(), 'manage_packages') OR public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'super_admin'));

DROP POLICY IF EXISTS "Anyone can view itineraries" ON public.package_itineraries;
CREATE POLICY "Anyone can view itineraries" ON public.package_itineraries FOR SELECT TO anon, authenticated
  USING (true);

DROP POLICY IF EXISTS "Content managers can edit itineraries" ON public.package_itineraries;
CREATE POLICY "Content managers can edit itineraries" ON public.package_itineraries FOR ALL TO authenticated
  USING (public.has_permission(auth.uid(), 'manage_packages') OR public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'super_admin'));

DROP POLICY IF EXISTS "Anyone can view media" ON public.media_library;
CREATE POLICY "Anyone can view media" ON public.media_library FOR SELECT TO anon, authenticated
  USING (true);

DROP POLICY IF EXISTS "Staff can manage media" ON public.media_library;
CREATE POLICY "Staff can manage media" ON public.media_library FOR ALL TO authenticated
...
DROP POLICY IF EXISTS "Admins/Managers can manage customers" ON public.customers;
CREATE POLICY "Admins/Managers can manage customers" ON public.customers FOR ALL TO authenticated
...
CREATE POLICY "Anyone can insert leads" ON public.leads FOR INSERT TO anon, authenticated
  WITH CHECK (true);

DROP POLICY IF EXISTS "Admins/Managers can manage leads" ON public.leads;
CREATE POLICY "Admins/Managers can manage leads" ON public.leads FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'super_admin') OR public.has_role(auth.uid(), 'manager'));

DROP POLICY IF EXISTS "Anyone can view settings" ON public.settings;
CREATE POLICY "Anyone can view settings" ON public.settings FOR SELECT TO anon, authenticated
  USING (true);

DROP POLICY IF EXISTS "Staff can manage settings" ON public.settings;
CREATE POLICY "Staff can manage settings" ON public.settings FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'super_admin') OR public.has_permission(auth.uid(), 'manage_content'));

DROP POLICY IF EXISTS "Anyone can view published posts" ON public.posts;
CREATE POLICY "Anyone can view published posts" ON public.posts FOR SELECT TO anon, authenticated
  USING (status = 'published' AND publish_date <= now());

DROP POLICY IF EXISTS "Staff can manage posts" ON public.posts;
CREATE POLICY "Staff can manage posts" ON public.posts FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'super_admin') OR public.has_permission(auth.uid(), 'manage_content'));

DROP POLICY IF EXISTS "Anyone can check coupons" ON public.coupons;
CREATE POLICY "Anyone can check coupons" ON public.coupons FOR SELECT TO anon, authenticated
  USING (is_enabled = true);

DROP POLICY IF EXISTS "Admins/Managers can manage coupons" ON public.coupons;
CREATE POLICY "Admins/Managers can manage coupons" ON public.coupons FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'super_admin') OR public.has_role(auth.uid(), 'manager'));

DROP POLICY IF EXISTS "Anyone can view approved reviews" ON public.reviews;
CREATE POLICY "Anyone can view approved reviews" ON public.reviews FOR SELECT TO anon, authenticated
  USING (status = 'approved');

DROP POLICY IF EXISTS "Anyone can submit reviews" ON public.reviews;
CREATE POLICY "Anyone can submit reviews" ON public.reviews FOR INSERT TO anon, authenticated
  WITH CHECK (true);

DROP POLICY IF EXISTS "Staff can manage reviews" ON public.reviews;
CREATE POLICY "Staff can manage reviews" ON public.reviews FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'super_admin') OR public.has_permission(auth.uid(), 'manage_content'));
...
DROP POLICY IF EXISTS "Anyone can view package gallery" ON public.package_gallery;
CREATE POLICY "Anyone can view package gallery" ON public.package_gallery FOR SELECT TO anon, authenticated
  USING (true);

DROP POLICY IF EXISTS "Staff can manage package gallery" ON public.package_gallery;
CREATE POLICY "Staff can manage package gallery" ON public.package_gallery FOR ALL TO authenticated
...
-- 20. STORAGE BUCKET CONFIGURATION & RLS POLICIES
...
-- Drop all old objects policies to prevent conflicts
...
-- Create robust public select policy for public buckets
CREATE POLICY "Public Select Objects" ON storage.objects FOR SELECT 
  USING (bucket_id IN ('packages', 'gallery', 'blog', 'media'));

-- Create simple, robust insert/update/delete policies for authenticated users
CREATE POLICY "Admin Insert Objects" ON storage.objects FOR INSERT TO authenticated 
  WITH CHECK (bucket_id IN ('packages', 'gallery', 'blog', 'documents', 'media'));

CREATE POLICY "Admin Update Objects" ON storage.objects FOR UPDATE TO authenticated 
  USING (bucket_id IN ('packages', 'gallery', 'blog', 'documents', 'media'));

CREATE POLICY "Admin Delete Objects" ON storage.objects FOR DELETE TO authenticated 
  USING (bucket_id IN ('packages', 'gallery', 'blog', 'documents', 'media'));


-- 21. Seed Initial Data
